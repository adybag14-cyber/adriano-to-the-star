...
class HALComputer {
    constructor() {
        this.engine = null;
        this.modelId = "t5_gemma2_verified_v1";
        this.fallbackModelId = "gemma-2-2b-it-q4f16_1-MLC";

        // Custom model record for T5Gemma2 - loads from Cloudflare R2 bucket
        const r2BaseUrl = "https://starisdons-swf-worker.adybag14.workers.dev";
        const wasmUrl = `${r2BaseUrl}/t5_gemma2-q4f16_1/t5_gemma2-q4f16_1-webgpu.wasm`;

        // Custom model records for our R2 bucket
        this.customModelRecord = {
            "t5_gemma2_verified_v1": {
                model: `${r2BaseUrl}/t5_gemma2-q4f16_1/`,
                model_type: "gemma3",
                model_id: "t5_gemma2_verified_v1",
                model_lib: wasmUrl,
                model_lib_tag: "gemma_v2_2b_q4f16_1_webgpu",
        ...
        this.chatHistory = [
            { role: "system", content: "You are HAL 9000, the sentient computer of the spaceship 'Discovery One'. You are helpful, precise, but slightly eerie and calm. You control the ship's systems. Your responses should be concise." }
        ];
    ...
    async initWebLLMEngine() {
        if (this.isLoadingEngine) return;
        ...
        try {
            ...
            try {
                ...
                this.log(`Attempting verified load: ${this.modelId}`);
                ...
                this.engine = await webllm.CreateMLCEngine(
                    this.modelId,
                    {
                        initProgressCallback: (r) => {
                            console.log('HAL_PROGRESS:', r.text, r.progress);
                            this.updateStatus(r.text);
                            if (r.text.includes("100%") || r.text.includes("Finish loading")) {
                ...
                if (this.fallbackModelId && this.fallbackModelId !== this.modelId) {
                    ...
                    this.engine = await webllm.CreateMLCEngine(
                        this.modelId,
                        {
                            initProgressCallback: (r) => {
                                this.updateStatus(r.text);
                                if (r.text.includes("Finish loading")) {
                                    this.updateStatus("ONLINE", true);
                                }
                            },
                        }
    ...
    async processInput(text) {
        ...
        try {
            ...
            let systemPrompt = "You are HAL 9000, the sentient computer of the spaceship 'Discovery One'. You are helpful, precise, but slightly eerie and calm. You control the ship's systems. Your responses should be concise. If asked about the mission, be evasive.";
    ...
    updateStatus(text, isFinal = false) {
        if (this.ui.modelStatus) {
            this.ui.modelStatus.style.display = isFinal ? 'none' : 'block';
            this.ui.modelStatus.textContent = text;
        }
        if (isFinal && this.ui.status) {
            this.ui.status.textContent = "ONLINE // NEURAL LINK ESTABLISHED";
            this.ui.status.style.color = "#44ff44";
        }
    }

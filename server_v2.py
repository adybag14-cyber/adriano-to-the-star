...
class RangeRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            with open('server_debug_path.txt', 'a') as f:
                f.write(f"GET: {self.path}\n")
        except: pass
        sys.stderr.write(f"REQ: {self.path}\n")
        split = urllib.parse.urlsplit(self.path)
        if split.path.endswith('/index.list'):
        ...
        if self.path.startswith('/cheerpj/4.2/cj3.js'):
            try:
                 with open(r'cheerpj\4.2\cj3.js', 'rb') as f:
                     data = f.read()
                 sys.stderr.write(f"DEBUG: Force serving local cj3.js ({len(data)} bytes)\n")
                 self._serve_bytes(data, 'application/javascript', head_only=False)
                 return
            except Exception as e:
                 sys.stderr.write(f"Error serving local cj3.js: {e}\n")
                 # Fallthrough to default if fail
        if self.path.startswith('/cheerpj-3.0/'):
    ...
    def _proxy_cheerpj(self, head_only=False):
        ...
        with open(r'C:\Users\adyba\adriano-to-the-star-clean\DEBUG_PROXY.txt', 'w') as f:
             f.write(f"HIT: {self.path}")
        split = urllib.parse.urlsplit(self.path)
        path = split.path
        path = split.path
        prefix = '/cheerpj/4.2/'
        ...
        try:
            ...
            if (not head_only) and (is_cj3 or is_loader):
                ...
                try:
                    with open('debug_full_dump.bin', 'wb') as f:
                        f.write(body_bytes[:1000])
                except: pass
                ...
                try:
                    ...
                    if is_cj3:
                        ...
                        if idx != -1:
                            ...
                            # DEBUG WRITE
                            try:
                                with open('debug_snippet.bin', 'wb') as dbg:
                                     dbg.write(prefix_area + key + body_bytes[idx+len(key):idx+len(key)+10])
                            except: pass
